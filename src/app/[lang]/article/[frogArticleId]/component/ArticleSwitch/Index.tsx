...
const ArticleSwitch=({frogArticleId,type,source}:Props)=>{
  ...
  return <div className="ArticleSwitch">
    <div onClick={()=>articleSwitch(lastNextMsg?.prepArticleId)} className="ArticleSwitch-last">
    {
        lastNextMsg?.prepArticleId&&<>
        <div className="action-text tstn">
          { t('上一篇') }
        </div>
    ...
    <div onClick={()=>articleSwitch(lastNextMsg?.nextArticleId)} className="ArticleSwitch-next">
    {
   ...
   {source!=='shopcart'&& <div className="ArticleSwitch-btn">
      <Button className="btn" type="primary" shape="round" size="large" onClick={toKaerumedia}>{ t('返回') }</Button>
    </div>}
  </div>
