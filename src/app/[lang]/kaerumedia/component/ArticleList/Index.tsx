...
import { FrogArticleRespDTO ,GetFrogArticleArchivePageDTO,GetFrogArticlePageDTO} from "@/service/customer";
...
const ArticleList=()=>{
  ...
  const { data:articleRes, loading, pagination, run:articlePage } = usePagination(
  ...
  useEffect(()=>{
    articlePage({pageSize:10,current:1},{frogArticleType:querys.tab===-1?undefined:querys.tab,stationCode,frogArticleYear:String(querys.year),frogArticleMonth:String(querys.month)})
  ...
  return <div className="ArticleList">
    <div className="ArticleList-main">
      <Spin spinning={loading}>
        {
         articleRes?.list?.length? articleRes.list.map((item: FrogArticleRespDTO)=>{
