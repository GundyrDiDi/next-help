        ...
        {list?.map((item, key) => {
          return (
            <div
              ...
              key={key}
              onClick={() => {
                const href = getLink(`media/${item.frogArticleId}`);
                if (!isLogin()) {
                  // 未登录 && 不可查看 的时候跳转 login
                  if (
                    item.noLoginRestriction ===
                    NO_LOGIN_RESTRICTION_TYPE.CHECK_DISABLE
                  ) {
                    toTheCkb(ENUM_PAGE.LOGIN, false);
                    return;
            ...
            >
              ...
              <div className="flex flex-col justify-between pc:ml-[16px] pad:ml-[16px] grow h-[100%] group ">
                <div className="text-[#333] pc:text-[18px] font-[700] text-[15px] line-clamp-2 mo:my-[12px] group-hover:!text-[--color-primary]">
                  {item.frogArticleTitle}
                </div>
                ...
                <div className="text-[#666] flex justify-between group-hover:!text-[--color-primary]">
                  ...
                  <div className="group-hover:!text-[--color-primary]">
                    {item.createTime}
                  </div>
                </div>
              </div>
