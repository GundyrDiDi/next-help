...
const list = [
  {
    title:
      "中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて",
  },
  {
    title:
      "中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて中国輸入ヤフオク販売のメリットと儲かる商品のリサーチ方法に…いて",
...
const HotArticalList = (props: HotArticalListProps) => {
  ...
  const { data } = useRequest(request.customer.frog.articleHotPage, {
    defaultParams: [{ pageNum: 1, pageSize: 12, stationCode }],
  });
  const { t } = useTranslation();
  return (
    <div className="bg-[#fff] rounded-[8px] pc:rounded-[8px] overflow-hidden">
      <div className="bg-[--fcolor] !text-[16px] text-[#fff] leading-[40px] h-[40px] px-[8px] flex items-center">
        {isShowHeaderIcon && <IconFlame className="mr-[8px]" />}
        {t("热门文章")}
      </div>
      {data?.data?.records?.map((item, idx) => (
        <a
          key={idx}
          className="flex text-[#666] text-[14px] px-[8px] py-[10px] cursor-pointer"
          href={`/${lang}/media/${item.frogArticleId}`}
          style={
            idx !== list.length - 1
