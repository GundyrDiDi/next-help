...
const Help = (props: HelpProps) => {
  ...
  const [searchResList, setSearchResList] =
    useState<SupportCenterContentDTO[]>();
  const [supportCenterSubjectList, setSupportCenterSubjectList] = useState(
    _supportCenterSubjectList
  );
  ...
  useEffect(() => {
    (async () => {
      const res = await request.customer.base.supportCenterTree({
        bizType:
          plat === ENUM_PLATE.b2b
            ? SupportCenterbizType.B
            : SupportCenterbizType.C,
  ...
  return (
    <div className="flex justify-center">
      <div className="hidden">
        <div>NEXT_PUBLIC_ENV{process.env.NEXT_PUBLIC_ENV}</div>
        <div>SERVER_ENV: {process.env.SERVER_ENV}</div>
      </div>
      {lang === "ja" ? <h1 className="hidden">THE CKBヘルプガイド</h1> : null}
      <div className="pc:w-[1200px] mo:w-[100%] pad:w-[100%]">
        ...
        {isShowSearchResult ? (
          <SearchResult
            ...
            onCloseSearch={() => setIsShowSearchResult(false)}
          />
        ) : (
          <div className="mo:p-[12px]">
                    ...
                    if (item?.contentList?.length) {
                      return (item.contentList ?? []).map((it) => {
                        return {
                          label: it.title!,
                          value:
                            it.supportCenterContentId! as unknown as string,
                          contentType: item.contentType as any,
                        };
                      });
                    }
