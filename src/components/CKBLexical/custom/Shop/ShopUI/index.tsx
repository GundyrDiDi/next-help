 ...
 * @LastEditTime: 2024-06-14 17:20:22
 * @Description: 
 */
...
const ShopItemUI = (props: ShopItemUI) => {
    ...
    return <div className={`p-[16px] rounded-[8px] border border-[#F0F0F0] box-border group cursor-pointer ${className}`} >
        <div className="mt-[8px] flex mb-[16px] items-center" >
            <img alt="" src={iconUrl} className="w-[24px] h-[24px]" />
            <div className="ml-[4px] line-clamp-1 text-[#1C2026] group-hover:text-[#008060] font-[700]" >{shopName}</div>
            <RightArrow className="ml-[4px] relative top-[-2px]" />
        </div>
        <div className="flex" >
            {productList?.map((item, key) => {
                return <div key={key} className={`w-[calc((100%-16px)/3)] ${key === 0 ? '' : 'ml-[8px]' }`} >
                    <img alt="" src={item.mainImgUrl} className="w-[100%] h-auto rounded-[4px]" />
...
const ShopUI = (props: ShopUIProps) => {
    ...
    return <>
        <div className="pc:flex pc:flex-wrap pc:w-[810px]" onClick={onClick} >
            {/* pc 端 店铺 > 2 的情况 */}
